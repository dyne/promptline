...
The embedded-skill contracts additionally verify that the catalog automatically discovers every top-level `SKILL.md`, exposes exact public bytes and startup metadata, retains operational scripts as read-only resources, excludes tests and Debian's validation-only scripts, and materializes one shared bundle license without per-skill copies. Command and MCP tests cover `list-skills`, `list-skill-files`, `materialize-skill`, initialize instructions, and raw `resources/list`/`resources/read` without creating instance state. App-server integration asserts the two required skill-isolation arguments exactly.
